...
<div id="sidebar"
    ...
    <nav class="flex-1 px-3 mt-4 overflow-y-auto no-scrollbar">
        <ul class="space-y-1">
            ...
            {{-- DASHBOARD (CORREGIDO): protegido por el mismo permiso que la ruta
                 (empresa.tablero), no por @role('admin'). Antes, un usuario con
                 el permiso pero sin ser "admin" literal podía entrar por URL sin
                 ver el link en el menú. --}}
            @can('empresa.tablero')
                <li>
            ...
            {{-- MÓDULO: COCINA / CHEF
                 ADVERTENCIA (sin corregir aquí, requiere cambio en routes/web.php):
                 la ruta 'orders.chef' NO tiene middleware 'can:...', así que aunque
                 el link se oculte con @role aquí, cualquier usuario logueado puede
                 entrar escribiendo /orders/chef directamente en el navegador.
                 Ver instrucciones debajo del archivo para el fix en routes/web.php. --}}
            @role(['admin', 'cocinero'])
                @php
            ...
            {{-- CORREGIDO: se agregó 'cajas.ver' a la condición del bloque completo
                 (antes solo 'ordenes.cobrar' y el typo 'gastos.index'), y también
                 se arregló el typo 'gastos.index' -> 'gastos.ver' más abajo. --}}
            @canany(['ordenes.cobrar', 'gastos.ver', 'cajas.ver'])
                @php
                    $isCashierActive =
                ...
                {{ $isCashierActive ? 'text-orange-600' : 'text-slate-400 group-hover:text-orange-600' }}"></i>
                    ...
                    <ul x-show="open && sidebarExpanded" x-cloak class="mt-1 ml-4 border-l-2 border-slate-100 space-y-1">
                        ...
                        {{-- CORREGIDO: 'Apertura y Cierre' apunta a boxes.index, cuya ruta
                             real exige 'can:cajas.ver' — antes decía 'ordenes.cobrar'. --}}
                        @can('cajas.ver')
                            <li>
