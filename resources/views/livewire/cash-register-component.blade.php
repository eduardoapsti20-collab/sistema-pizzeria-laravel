                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="text-slate-400 text-[10px] uppercase tracking-[0.15em] bg-slate-50/50">
                            ...
                            <th class="px-8 py-5 font-bold">Apertura</th>
                            <th class="px-8 py-5 font-bold">Cierre</th>
                            ...
                            <tr class="hover:bg-slate-50/80 transition-all group">
                                ...
                                <td class="px-8 py-5">
                                    <p class="text-xs font-semibold text-slate-600 flex items-center gap-1.5">
                                        ...
                                        {{ ($reg->opened_at ?? $reg->created_at)->format('d/m/Y') }}
                                    </p>
                                    <p class="text-[10px] text-slate-400 font-medium mt-0.5 pl-4.5">
                                        {{ ($reg->opened_at ?? $reg->created_at)->format('h:i A') }}
                                    </p>
                                </td>

                                {{-- FECHA/HORA DE CIERRE (NUEVO) --}}
                                <td class="px-8 py-5">
                                    @if ($reg->closed_at)
                                        <p class="text-xs font-semibold text-slate-600 flex items-center gap-1.5">
                                            <i class="fa-regular fa-calendar-check text-rose-400 text-[10px]"></i>
                                            {{ $reg->closed_at->format('d/m/Y') }}
                                        </p>
                                        <p class="text-[10px] text-slate-400 font-medium mt-0.5 pl-4.5">
                                            {{ $reg->closed_at->format('h:i A') }}
                                        </p>
                                    @else
                                        <span class="text-[10px] text-slate-300 font-semibold italic">— Sigue
                                            abierta —</span>
                                    @endif
                                </td>
