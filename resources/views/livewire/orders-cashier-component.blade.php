<div class="p-6 bg-[#fcfcfc] min-h-screen font-sans text-slate-900 antialiased">
                ...
                <div wire:key="order-{{ $order->id }}"
                    ...
                    <div class="p-4 border-b border-slate-50 bg-slate-50/50 flex justify-between items-center">
                        <div>
                            ...
                            <h2 class="text-2xl font-black leading-none text-slate-800">{{ $order->table->name ?? 'N/A' }}</h2>
                        </div>
    ...
    <div
        class="group relative bg-white p-3 rounded-2xl border border-slate-100 shadow-sm transition-all">
    ...
    <span
        class="ml-1.5 text-[10px] font-bold text-slate-400 peer-checked:text-orange-600 uppercase whitespace-nowrap">Cobrar</span>
</label>
        </div>
        ...
        <div class="flex items-center justify-between gap-3">
            <div class="flex items-start gap-3 min-w-0 flex-1">
                ...
                <div class="min-w-0 flex-1">
                    <p class="text-sm font-black text-slate-800 uppercase leading-snug">
                        {{ $detail->product->name ?? 'Producto eliminado' }}
                        @if ($detail->productSize)
                            <span class="text-orange-600 whitespace-nowrap">({{ $detail->productSize->name }})</span>
                        @endif
                    </p>
                    <p class="text-[11px] font-bold text-slate-500 mt-0.5">
                        {{ $empresa->currency_simbol ?? 'S/' }}{{ number_format($detail->price, 2) }}
                    </p>
                    @if ($detail->notes)
                        <p
                    ...
                    <div class="p-4 bg-slate-50/80 border-t border-slate-50 mt-auto">
                        <div class="flex justify-between items-center">
                            <span class="text-[9px] font-bold text-slate-400 uppercase tracking-tighter">
                                {{ $order->order_code }} | {{ $order->user->name ?? 'Sistema' }}
                            </span>
                            <span class="text-sm font-black text-slate-900">
                                {{ $empresa->currency_simbol ?? 'S/' }}{{ number_format($order->amount_pending, 2) }}
                            </span>
                        </div>
        ...
        <div class="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 transition-all">
            ...
            <div
                ...
                <div class="p-6 border-b border-slate-100 flex justify-between items-center bg-white">
                    <div>
                        ...
                        <p class="text-3xl font-black text-slate-800 tracking-tight">
                            <span
                                class="text-orange-600 font-light">{{ $empresa->currency_simbol ?? 'S/' }}</span>{{ number_format($paymentAmount, 2) }}
                        </p>
                    </div>
                    ...
                    <div class="flex divide-x divide-slate-100 border-b border-slate-100 bg-white">
                        ...
                        <div class="flex-1 p-4 text-center">
                            ...
                            <span class="text-lg font-bold text-slate-700">
                                {{ $empresa->currency_simbol ?? 'S/' }}{{ number_format($this->paid, 2) }}
                            </span>
                        </div>
                        ...
                        <div class="flex-1 p-4 text-center">
                            @if ($this->change > 0)
                                ...
                                <span class="text-lg font-bold text-orange-600">
                                    {{ $empresa->currency_simbol ?? 'S/' }}{{ number_format($this->change, 2) }}
                                </span>
                            @else
                                <span class="block text-[10px] uppercase font-bold text-slate-400">Faltante</span>
                                <span class="text-lg font-bold text-rose-500">
                                    {{ $empresa->currency_simbol ?? 'S/' }}{{ number_format(max(0, $paymentAmount - $this->paid), 2) }}
                                </span>
                            @endif
                        </div>
                ...
                <div class="p-6 bg-slate-50 border-t border-slate-200">
                    ...
                    {{-- CORREGIDO: se quitó el ícono con animate-spin, que giraba
                         indefinidamente y hacía parecer que la pantalla estaba
                         "cargando" cuando en realidad solo faltaba dinero por cobrar. --}}
                    <button wire:click="processPayment" wire:loading.attr="disabled" wire:target="processPayment"
                        @if ($this->paid < $paymentAmount) disabled @endif
                        ...
                        @if ($this->paid < $paymentAmount)
                            ...
                            Faltan {{ $empresa->currency_simbol ?? 'S/' }}{{ number_format($paymentAmount - $this->paid, 2) }}
                        @else
