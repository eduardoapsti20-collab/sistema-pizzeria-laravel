                    ...
                    <div
                        ...
                        <div class="p-3">
                            ...
                            @if ($product->sizes->isNotEmpty())
                                {{-- PRODUCTO CON VARIOS TAMAÑOS: un botón por tamaño, cada uno con su precio --}}
                                <div class="space-y-1.5">
                                    @foreach ($product->sizes as $size)
                                        <button wire:click="addToOrder({{ $product->id }}, {{ $size->id }})"
                                            class="w-full flex items-center justify-between px-2.5 py-1.5 bg-slate-50 hover:bg-orange-50 border border-slate-100 hover:border-orange-300 rounded-lg transition-all active:scale-95 group/size">
                                            <span
                                                class="text-[11px] font-bold text-slate-600 group-hover/size:text-orange-700 uppercase tracking-tight">
                                                {{ $size->name }}
                                            </span>
                                            <span class="text-[11px] font-black text-slate-900">
                                                {{ $empresa->currency_simbol }}{{ number_format($size->price, 2) }}
                                            </span>
                                        </button>
                                    @endforeach
                                </div>
                            @else
                                {{-- PRODUCTO CON PRECIO ÚNICO: comportamiento de siempre --}}
                                    ...
                                    <div class="flex justify-between items-start mb-1">
                                        <div class="flex-1">
                                            ...
                                            @if (!empty($item['size_name']))
                                                <span
                                                    class="text-[9px] font-bold text-orange-500 uppercase tracking-wider">
