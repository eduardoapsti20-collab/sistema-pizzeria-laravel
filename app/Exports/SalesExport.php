...
class SalesExport implements FromCollection, WithHeadings, WithStyles, WithCustomStartCell, WithMapping, ShouldAutoSize
{
    ...
    public function collection()
    {
        return Sale::with(['order.table', 'order.user'])
            ->when(!empty($this->request['from']), fn($q) => $q->whereDate('created_at', '>=', $this->request['from']))
->when(!empty($this->request['to']), fn($q) => $q->whereDate('created_at', '<=', $this->request['to']))
...
{
    return ['CÓDIGO VENTA', 'FECHA/HORA', 'MESA', 'ATENDIDO POR', 'TOTAL'];
}
